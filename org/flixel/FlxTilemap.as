package org.flixel {
	...
	public class FlxTilemap extends FlxObject
	{
		...
		_pathfindingNodes = [];
			for (var j : int = 0; j < totalTiles; j++) {
				_pathfindingNodes.push(new FlxPathfindingNode(j % widthInTiles, int(j / heightInTiles)));
			}
		...
		//--pathfinding extracted from FlxPathfinding--
		private var _pathfindingNodes : Array;
		...
		private function getNodeAt(x : int, y : int) : FlxPathfindingNode {
			return _pathfindingNodes[x + y * widthInTiles];
		}
		
		private function isWalkable(node:FlxPathfindingNode):Boolean{
			return getTileByIndex(node.y * widthInTiles + node.x) < collideIndex;
		}
